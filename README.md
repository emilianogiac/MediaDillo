...
- **TMDB / TVDB metadata matching** — TMDB for movies and show-level metadata (artwork, overview, credits); TVDB is the authoritative episode source for TV shows — when a TVDB ID is available the enricher uses a single bulk TVDB fetch for all seasons, bypassing TMDB's episode numbering entirely (fixes shows like Columbo where TMDB and TVDB episode counts diverge); TVDB ID resolved automatically via TMDB external_ids on first enrich — no separate match step needed; IMDb → TMDB auto-match via external ID lookup; Italian (or any language) metadata via `METADATA_LANGUAGE` env var; **per-show TVDB episode ordering** — dropdown on the show detail page shows only the orderings TVDB actually provides for that series (Aired, DVD, Absolute); selecting one saves it and immediately re-enriches with the new ordering; Absolute ordering (important for anime) fetches all episodes flat without season filtering; ordering dropdown is hidden if TVDB only offers one ordering; changing the ordering dropdown triggers an automatic re-enrich so the new structure is applied right away; **TVDB-only shows** — the Match modal on show detail pages includes a TVDB tab for shows not available on TMDB; search TVDB by title or enter a TVDB ID directly; matched shows are marked with a "TVDB only" badge and refresh entirely from TVDB (title, overview, year, poster, episodes) on subsequent Re-match calls; movies always use TMDB only; TVDB authentication failures surface as a clear 422 "check your TVDB_API_KEY" error instead of a generic 500; **TVDB language support** — episode titles and search results from TVDB respect the `METADATA_LANGUAGE` setting; fallback chain is primary lang → eng → null (the bare/unpredictable TVDB default endpoint is not used, preventing random French/German titles from appearing for shows with incomplete translations)
...
- **TV Shows list — sort & filter** — full filter parity with movies: library selector (scan root tabs/dropdown), genre picker (client-side), sort by title, year, rating, or episode completeness, quality tier filter, Airing/Ended status toggles, New badge and filter, Missing artwork, Unmatched, Needs organizing, and Duplicates filters; active filters shown as dismissible chips; grid and list views with completeness bar; **batch actions in list view** — Rematch, Rename all (background job), Cleanup (removes stale-extension files from show folders), and Remove records; completeness bar colors (green = 100%, blue ≥ 80%, yellow ≥ 50%, red < 50%); **multi-library split** — shows belonging to more than one scan root appear as a separate row per library so each instance is visible and independently selectable; **list view details** — each row shows the library label, quality/codec badges, and an "⚠ Organize" badge when the folder needs renaming; **deselect on filter** — selected items that are filtered out of the visible list are automatically deselected; **Watch in Jellyfin ↗** deep-link button on show detail page header — tries TMDB provider lookup first, falls back to TVDB so TVDB-only shows open correctly too; button visible for any matched show (TMDB or TVDB); show detail header displays the library badge(s) the show belongs to; **season detail episode list** — episodes beyond the TVDB-authoritative count that have no file are hidden (stale phantom records); episodes beyond the count that do have a file are highlighted orange; a "Hide/Show missing" toggle controls visibility of missing episodes (default: shown); the owned/total counter uses the TVDB episode count as the denominator
...
- **Activity log** — dedicated `/activity` page showing a reverse-chronological log of every action across the library: renames, cleanup, scans, files added/removed, collection moves, episode merges and assignments; filter by action type; multi-select revert for rename and move entries (swaps files back and updates DB records); 180-day retention; migrated from the previous movie-only rename history; **scanner deduplication fix** — show records no longer created per-season when scan root path has a trailing slash; season-scan root derivation now uses a DB lookup instead of fragile double-dirname
- **Rename undo** — rename history visible in the Activity log; multi-select revert supported for both movie and episode renames
...
- **Editable API keys** — TMDB, TVDB, Jellyfin URL/key, and metadata language are configurable from Settings → API Keys in the UI; values are validated against their respective APIs before saving; env var values are seeded on first start and continue working as fallbacks; **Cleanup Library** button in Settings removes stale DB records — items whose files are missing from disk, items belonging to libraries that are no longer configured, and ghost movies/shows with no remaining files; result shows counts for movies removed, shows removed, and episodes marked missing
