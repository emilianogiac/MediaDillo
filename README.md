...
- **Activity log** — dedicated `/activity` page showing a reverse-chronological log of every action across the library: renames, cleanup, scans, files added/removed, collection moves, episode merges and assignments; filter by action type; multi-select revert for rename and move entries (swaps files back and updates DB records); 180-day retention; migrated from the previous movie-only rename history
