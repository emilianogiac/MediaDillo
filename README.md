...
- **Match vs Re-match** — "Match" opens TMDB search (with inline title search input) to assign a new item; "Re-match" silently re-enriches the show using the existing match (fetches fresh episode data from TVDB if configured, artwork from TMDB) with no modal; artwork downloads automatically on every match/rematch; batch Re-match runs as a background job with a persistent progress toast that lists failing items by title
